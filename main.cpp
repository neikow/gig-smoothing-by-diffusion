...
std::vector<std::vector<float> > laplace_beltrami_matrix(Mesh &mesh) {
    ...
    for (Mesh::VertexIter vv_it_i = mesh.vertices_begin(); vv_it_i != mesh.vertices_end(); ++vv_it_i) {
        ...
        for (Mesh::VertexVertexIter vv_it_j = mesh.vv_iter(vi); vv_it_j.is_valid(); ++vv_it_j) {
            ...
            auto [aij, bij] = compute_cotan_weight_for_vertices(mesh, vi, vj);
...
std::string algorithm_to_string(const Algorithm algo) {
    switch (algo) {
        ...
        case LAPLACE_BELTRAMI_MATRIX_BIS:
            return "laplace_beltrami_matrix_bis";
    }
    return "unknown";
}
...
void square_matrix_iterative_smoothing(Mesh &mesh, const float lambda) {
    ...
    for (Mesh::VertexIter v_it = mesh.vertices_begin(); v_it != mesh.vertices_end(); ++v_it) {
        ...
        for (Mesh::VertexVertexIter vv_it = mesh.vv_iter(vhi); vv_it.is_valid(); ++vv_it) {
            ...
            auto [aij, bij] = compute_cotan_weight_for_vertices(mesh, vhi, vhj);
    ...
    auto alg_func = [&](Mesh &mesh, const float lambda) {
        ...
        switch (algo) {
            ...
            case COTANGENTIAL_LAPLACE_BELTRAMI:
                ...
                break;
            case LAPLACE_BELTRAMI_MATRIX:
                square_matrix_iterative_smoothing(copy, lambda);
                break;
            case LAPLACE_BELTRAMI_MATRIX_BIS:
                square_matrix_iterative_smoothing_bis(copy, lambda);
        }
    };
